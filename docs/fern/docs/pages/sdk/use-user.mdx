...
Fetches the current user or internal user based on provided options.

## Parameters

- `options`: `GetUserOptions` (optional) - Options for fetching the user, which may include criteria for project ID matching.

## Returns

`CurrentUser | CurrentInternalUser | null` - Returns the current user, internal user, or null if no user is found.
